...
10. **Secret redaction rule**: When you encounter credentials, passwords, tokens, API keys, private key material, or password hashes, report only their *presence*, *location*, and *file permissions* — never quote the actual secret values in findings, database records, or report output.
11. **Follow your nose**: if anything looks interesting or unusual, dig deeper. Check config files, look at what's actually in docker containers, read systemd unit files for custom services. List commands and surface commands only tell you what exists — reading the actual files tells you what things *do*.
...
Insert or update the server's entry. Use `ON CONFLICT` to update only the columns this skill owns, so a prior security-explore scan's data is preserved:

```sql
INSERT INTO server_analysis (hostname, purpose, analysis)
